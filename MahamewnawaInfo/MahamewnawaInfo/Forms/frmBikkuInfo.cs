...
namespace MahamewnawaInfo.Forms
{
    public partial class frmBikkuInfo : DevComponents.DotNetBar.Office2007Form
    {
        ...
        private void SetBInfoFromFields(BikkuInfo bInfo)
        {
            ...
            bInfo.CurrentStatus = (DBCore.CurrenStatus)(sitiRadio.Checked ? 1 : otherPlaceRadio.Checked ? 2 : 3);
        ...
        private void SetBhikkuFields(BikkuInfo bInfo)
        {
            ...
            {
                setBhikkuPictureFromByteArray(Utility.GetByte64String(bInfo.ImageData));
            }
        ...
        private void setPictureFromFile(string fileName)
        {
            try
            {
                ...
                pictureBox.Image = Utility.getThumbImage(img, pictureBox.Width, pictureBox.Height);
        ...
        private void setBhikkuPictureFromByteArray(byte[] picData)
        {
            try
            {
                ...
                if (isValiedFroToDate(histry))
                {
                    ...
                    if (bhikkuID != 0)
                    {
                        using (BikkuInfo info = new BikkuInfo(true))
                        {
                            ...
                            if (histryID == 0)
                            {
                                info.AddAsapuHistry(histry.AsapuID, histry.FromDate, histry.ToDate, histry.Post, histry.Note);
                            }
                            else
                            {
        ...
        private void addInfoBtn_Click(object sender, EventArgs e)
        {
            if (validateOtherData())
            {
                ...
                AddOtherDataRow(0, descriptionTxt.Text, fileName, Utility.GetFileByteCompress(imageName.Text));
